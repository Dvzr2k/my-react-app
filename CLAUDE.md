...
React 19 app (create-react-app) serving as a DevOps capstone portfolio, deployed to AWS using S3 + CloudFront, provisioned with Terraform, and automated via GitHub Actions with Claude Code as the AI orchestration engine.

## Architecture

### Application (React App)
- **src/App.js** — Main component: author card, tech stack badges, infra grid, project description (inline JS styles)
- **src/index.js** — Entry point, renders `<App />` into `public/index.html`'s `<div id="root">`
- **src/App.css / src/index.css** — Global base styles (most styling is inline in App.js)
- **public/index.html** — HTML shell template; browser never sees this directly — React injects into it
- **src/tests/App.test.js** — Test suite (run with `npm test`)
- React 19 + create-react-app; requires `npm run build` before deploying — outputs production-ready static files to `build/`
...
- Runs `npm ci && npm run build` to install dependencies and compile React app into `build/`
- Syncs `build/` directory to S3, then invalidates CloudFront cache
...
/deploy                              → Build React app (npm run build), sync build/ to S3 + invalidate CloudFront
...
npm start      # Run this every time you code
...
# On VMware shared folder (hgfs) only — use these instead of the two lines above
